  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const wave1 = Math.sin(elapsedTime * 0.5 + positions[i3] * 0.05);
        const wave2 = Math.cos(elapsedTime * 0.3 + positions[i3 + 1] * 0.05);
        const wave3 = Math.sin(elapsedTime * 0.7 + positions[i3 + 2] * 0.05);
        
        positions[i3] += wave1 * 0.02;
        positions[i3 + 1] += wave2 * 0.02;
        positions[i3 + 2] += wave3 * 0.01;

        // Enhanced mouse interaction
        const particlePos = new THREE.Vector3(positions[i3], positions[i3 + 1], positions[i3 + 2]);
        const mousePos = new THREE.Vector3(mouse.x * 25, mouse.y * 25, 0);
        ...
        if (distanceToMouse < 8) {
          ...
          const force = (8 - distanceToMouse) / 8 * 0.3;
